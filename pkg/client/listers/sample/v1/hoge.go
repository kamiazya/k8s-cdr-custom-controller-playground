...
type HogeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Hoge, err error)
...
// List lists all Hoges in the indexer.
func (s *hogeLister) List(selector labels.Selector) (ret []*v1.Hoge, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Hoge))
	})
	return ret, err
}
...
// List lists all Hoges in the indexer for a given namespace.
func (s hogeNamespaceLister) List(selector labels.Selector) (ret []*v1.Hoge, err error) {
...
// Get retrieves the Hoge from the indexer for a given namespace and name.
func (s hogeNamespaceLister) Get(name string) (*v1.Hoge, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("hoge"), name)
	}
	return obj.(*v1.Hoge), nil
}
